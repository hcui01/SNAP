...
public class N_F_EXP extends Policy {
	
	public N_F_EXP () {
		super();
	}
	
	public N_F_EXP(String instance_name) {
		super(instance_name);
		Global.ifLift = false;
		searchDepth = -1;
			...
			if(searchDepth == -1){
				...
				if(F.counter != 0) {
					if (!ifFirstStep) {
						...
						if ((numOfNodes + nextStepNodesNum) * updatePerNode * 500 * Math.pow(1.5, currentRealConformant - 1)> timeLeft) {
