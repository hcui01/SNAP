...
public class Y_F extends Policy {
	...
	public ExpandResults ExpandForAObs(TEState as, int startH, int endH, int obsIndex, double discount, TreeExp F,
		...
		counterRecord.add(F.counter - lastStepCounter);
		//System.out.println(counterRecord.get(counterRecord.size() - 1));
		...
		for (; h < endH; h++) {
			...
			if (searchDepth == -1) {
				...
				if (F.counter != 0) {
					...
					double estimateAll = obsIndex < (countObs / 3) ? (countObs / 3) : countObs;
					if ((numOfNodes + nextStepNodesNum) * estimateAll / (obsIndex + 1.0) * updatePerNode
							* 500 > timeLeft) {
						h++;
						break;
					}
	...
	public void UpdateBelief(State newPassIn) throws EvalException {
		...
		if(ifInitialBelief) {
			...
			belief.advanceNextState();
			System.out.println(belief._state);
		}
			...
			for(int r = 0; r <= countObsVars; r ++) {
			    ...
			    while (iterator.hasNext()) {
			        ...
			        if(!obsProb.Is0()) {
			        	...
			        	F = ExpandForAObs(belief, 1, -1, obsIndex, instance._dDiscount, F, treeCounter, nextStepNodesNum, updatePerNode, counterRecord, qEst, false, obsProb).qGraph;
