...
public class Y_F_Sample extends Policy {
	
	public Y_F_Sample () {
		super();
	}
	
	public Y_F_Sample(String instance_name) {
		super(instance_name);
		Global.ifLift = true;
		...
		for (; h < endH; h++) {
			...
			if (searchDepth == -1) {
				...
				//if (F.counter != 0) {
					...
					if ((numOfNodes + (sumOfOneStepSize+1) / (oneStepSizeCounter+1)) * maxNumObs / (obsIndex + 1.0) * updatePerNode
							* 200 > timeLeft) {
				...
				if(countObs == 0) {
					...
					for(int r = 0; r <= countObsVars; r ++) {
					    ...
					    while (iterator.hasNext()) {
					        ...
					        obsIndex ++;
					        treeCounter.Count(F);
					        if ((treeCounter.nonLeafCounter + (sumOfOneStepSize+1) / (oneStepSizeCounter+1)) * updatePerNode
									* 200 > _timeAllowed - (System.currentTimeMillis() - t0)) {
								maxNumObs = obsIndex + 1;
								countObs = obsIndex + 1;
								break;
							}
					    }
